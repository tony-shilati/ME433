...
int main()
{
    ...
    uint slice_num = pwm_gpio_to_slice_num(GPIO_FUNC_PWM); // Get PWM slice number
    ...
    pwm_set_gpio_level(MOTOR1_EN, wrap * 0); // set the duty cycle to 0
    ...
    while (true) {
        ...
        case (cmd) {
            case '+':
                ...
                if (speed > 1.0f) speed = 1.0f; // Cap at 100%
                break;
            case '-':
